...
export default async function RootLayout({
  ...
  // Container widths
  if (settings.container_width_header) themeVars["--container-header"] = `${settings.container_width_header}px`
  ...
  // Header height (based on logo height + padding, min 64px)
  const logoH = settings.logo_height ? Number(settings.logo_height) : 0
  themeVars["--header-height"] = logoH ? `${Math.max(logoH + 24, 64)}px` : "64px"
