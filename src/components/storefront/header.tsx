        ...
        return (
          <div key={item.id} className="group relative">
            <span className="flex cursor-pointer items-center gap-1">
              ...
              <svg className="h-3 w-3 text-[var(--header-nav-color)] transition-transform group-hover:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export async function Header() {
  ...
  const [cartCount, settings, headerMenuItems] = await Promise.all([
    getCartCount(),
    getSettings(["store_name", "site_logo_url", "logo_height", "header_layout", "enable_wishlist"]),
    getMenuItems("header"),
  ])
